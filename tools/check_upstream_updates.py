...
def upstream_slug(repo: str, repo_dir: Path | None = None) -> str | None:
    """`https://github.com/owner/name.git` -> `owner/name`, or None if not GitHub.

    This fork's baseline names a git *remote* (`upstream`) rather than a URL,
    because the scheduled workflow adds that remote from the GitHub fork parent
    at run time. A remote name carries no owner or repository, so resolve it
    through git first -- otherwise the ticket axes fail closed forever and the
    report says "not checked" every single week, which is true but useless.
    """
    match = re.search(
        r"github\.com[:/](?P<owner>[^/]+)/(?P<name>[^/]+?)(?:\.git)?$", repo
    )
    if match:
        return f"{match['owner']}/{match['name']}"
    if repo_dir is None or "/" in repo or ":" in repo:
        return None
    try:
        url = run_git(["remote", "get-url", repo], repo_dir).strip()
    except UpstreamCheckError:
        return None
    return upstream_slug(url) if url else None


def collect_new_tickets(
    baseline: dict, kind: str, repo_dir: Path | None = None
) -> list[dict] | None:
    ...
    """
    slug = upstream_slug(str(baseline["repo"]), repo_dir)
    if not slug:
        return None
...
def main() -> int:
    ...
    try:
        ...
        prs = collect_new_tickets(baseline, "pr", args.repo_dir)
        issues = collect_new_tickets(baseline, "issue", args.repo_dir)
